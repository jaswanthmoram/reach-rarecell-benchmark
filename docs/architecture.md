...
Predictions (CSV)
    ...
    │
    ├──► src/rarecellbenchmark/evaluate/metrics.py
...
- **Inputs:** `pyproject.toml` (pip install -e '.[dev]'), `setup/renv/renv.lock` (R package management).
...
- **Scale:** 160 units (8 datasets × 4 tiers × 5 replicates, with T4 limited to datasets with sufficient background cells).
...
- **Scale:** 640 units (4 noise conditions × 140 Track A units, with optional 100 additional panel units).
...
- **Figures (11 total: 8 data-driven + 3 schematics):**
...
- **Outputs:** `data/results/figures/phase12/*.png` (PNG previews), `data/results/manuscript/figures/*.pdf` (publication-quality PDFs)
...
3. Update `run_all.py`, `src/rarecellbenchmark/evaluate/metrics.py`, and `scripts/generate_figures.py`.

**New metric:**
1. Add metric function to `src/rarecellbenchmark/evaluate/metrics.py`
...
- **Software versions:** Pinned in `requirements/base.txt`, `requirements/full.txt`, `requirements/dev.txt`, and `requirements/r.txt`.
