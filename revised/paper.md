...
| crc_lee | Colorectal cancer | 10x Chromium | A/B/C/E/F | 55,551 |
...
- **Track F (intra-lineage, crc_lee only):** Distinguished from inter-lineage tracks, Track F evaluates the fundamentally harder task of separating malignant from normal cells of the same lineage. See the dedicated "Intra-Lineage Evaluation (Track F)" section below for the detailed leaderboard and the scMalignantFinder pretrained supervised ceiling.
...
## Intra-Lineage Evaluation (Track F)

Track A–E performance is built on the inter-lineage malignant-vs-TME contrast (e.g., epithelial tumor cells vs. stromal or immune cells) and does not imply the ability to distinguish malignant from normal cells of the *same* lineage (intra-lineage setting). Track F evaluates this harder task using the colorectal cancer dataset `crc_lee` where malignant epithelial cells must be surfaced above an abundant normal colorectal epithelial background.

Track F includes `scMalignantFinder` as a **pretrained supervised ceiling reference** (Yu et al., 2025) rather than a ranked unsupervised competitor. Because `scMalignantFinder` is a supervised deep-learning model pretrained on thousands of malignant and normal epithelial cells, it has learned complex lineage-specific somatic and expression features that are completely hidden from unsupervised or naive methods. As shown in the Track F leaderboard below, only `scMalignantFinder` retains discriminative power across all three prevalence tiers.

### Track F Leaderboard (crc_lee only)

| Method | 0.1% Median AP | 0.5% Median AP | 1.0% Median AP | Role |
|--------|----------------|----------------|----------------|------|
| scMalignantFinder | 1.000 | 1.000 | 0.917 | Pretrained supervised ceiling |
| expr_threshold | 0.008 | 0.062 | 0.154 | Naive baseline |
| CaSee | 0.003 | 0.049 | 0.063 | Exploratory |
| FiRE | 0.004 | 0.022 | 0.040 | Published |
| cellsius | 0.001 | 0.007 | 0.008 | Published |
| random_baseline | 0.002 | 0.008 | 0.016 | Floor |
| hvg_logreg | 0.001 | 0.005 | 0.010 | Floor (collapse) |
| DeepScena | 0.001 | 0.005 | 0.011 | Published |
| RareQ | 0.001 | 0.005 | 0.010 | Published |
| scCAD | 0.001 | 0.005 | 0.011 | Published |

At the extremely low prevalence of 0.1%, all ranked published and unsupervised methods collapse completely to the chance floor (AP ≤ 0.009), showing that inter-lineage leaderboard results cannot be extrapolated to intra-lineage sub-state detection. Even the in-sample supervised `hvg_logreg` ceiling (which achieves AP = 1.000 on Track A) collapses to the chance floor (0.001) because it relies on highly variable genes (HVGs) which do not separate normal from malignant cells of the same lineage.

**Two critical caveats (`data/results/revision/VERIFICATION_ISSUES.md`):**
1. **Cancer-type specificity:** Track F is currently evaluated only on `crc_lee` because no other REACH dataset has normal epithelial reference annotations. Since `scMalignantFinder`'s Track A performance varies widely across cancers—ranging from AP = 0.015 (hnscc_puram) to 0.943 (crc_lee)—its high Track F AP is colorectal-cancer-specific and **must not** be generalized.
2. **Background duplication:** At 0.1% prevalence (2,000-cell units), `crc_lee`'s 340 available normal epithelial cells produce 83% duplicate background IDs; at 0.5% and 1.0% prevalence (400-cell units), duplication drops to 14.6% and 14.1% respectively, well under the 20% design cap. While this duplication slightly inflates absolute AP, it affects all methods identically, keeping relative comparisons valid.
...
| Tracks | 6 (spike-in, synthetic, null, natural, noise, intra-lineage) | Multi-metric per task | Per-task metrics |
